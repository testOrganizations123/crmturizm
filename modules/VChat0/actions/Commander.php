...
class VChat_Commander_Action extends Vtiger_Action_Controller{
	...
	private function updateDialog($channelto,$userto)
	{
		//$channelto=$_REQUEST["channelto"];
			...
			if ($_REQUEST["channelto"]>0)
			{
				...
				if($adb->num_rows($result2) > 0) {
					...
					if ($subid>0)
					{
						...
						$this->updateDialog($subid,0);
						
					}
				}
					   
			}
			/* vtiger_vchat_text ssssssssssss */
		}
		/*
		//$channelto=$_REQUEST["channelto"];
		//$userto=$_REQUEST["userto"];
		*/
		$this->updateDialog($_REQUEST["channelto"],$_REQUEST["userto"]);
		$result2 = array('success'=>true, 'message'=>'!!!!!!','returnid'=>'1');
		$response = new Vtiger_Response();
		$response->setResult($result2);
		$response->emit();
	...
	public function uploadsubdata()
	{
		...
		if($adb->num_rows($resultchannels) > 0) {
			...
			$countusers=$adb->num_rows($resultchannels);
		}
		else
		{
			$countusers=0;
		...
		$result2 = array('success'=>true, 'message'=>'!!!!!!','returnid'=>'1','htmluser'=>$masChannel,'htmlchannels'=>$masSubChannel,'sqls'=>$sqlsubchannels,'countchannels'=>$countchannels,'countusers'=>$countusers,'homeid'=>$homeid,'homename'=>$homename);
		$response = new Vtiger_Response();
		$response->setResult($result2);
		$response->emit();
		/**/
