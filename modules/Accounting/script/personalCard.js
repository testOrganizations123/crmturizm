...
webix.ready(function () {
    ...
    var dtableRate = new webix.ui({
        ...
        width: 1000,
        columns: [
            {id: "cost_hours", header: "сумма / час, руб", width: 280, editor: "text"},
        ],
        ...
        on: {
            onAfterEditStop: function (cell, coordinates) {
                ...
                var worker = window.userInfo.user_id;
                var value = cell.value.replace(/ /g, "");
                value = value.trim();

                if ((value && parseInt(value) != value) || parseInt(value) < 0) {
                    $(function () {
                        new PNotify({
                            title: 'Error!',
                    ...
                    var record = dtableRate.getItem(coordinates.row);
                ...
                $.ajax({
                    type: "GET",
                    url: "/index.php?module=Accounting&view=List&mode=editWorkers&value=" + value + "&column=" + column + "&user=" + worker,
    ...
    var dtableLevels = new webix.ui({
        container: "levels",
        view: "datatable",
        width: 1000,
        columns: [
            {id: "percent1", header: "1 этап, %", width: 188, editor: "text"},
            {id: "percent2", header: "2 этап, %", width: 188, editor: "text"},
            {id: "percent3", header: "3 этап, %", width: 188, editor: "text"},
            {id: "percent4", header: "4 этап, %", width: 188, editor: "text"}
        ],
        ...
        on: {
            onAfterEditStop: function (cell, coordinates) {
                ...
                if ((value && parseInt(value) != value) || parseInt(value) < 0) {
                    ...
                    var record = dtableLevels.getItem(coordinates.row);
                    record[coordinates.column] = cell.old;
                    dtableLevels.refresh();
    ...
    for (var yearVacations in window.vacationArchive) {
        var regularVacation = new webix.ui({
            container: "vacation_" + yearVacations,
            view: "datatable",
            width: 1000,
            columns: [
            ...
            autowidth: true,
            editable: false,
            data: window.vacationArchive[yearVacations]
        });
    }

    for (var yearVacationsAt in window.vacationPromotionalArchive) {
        var regularVacationAt = new webix.ui({
            container: "vacation_advertising_tour_" + yearVacationsAt,
            view: "datatable",
            width: 1000,
            columns: [
                {id: "start", header: "Начало", width: 110},
            ...
            autowidth: true,
            editable: false,
            data: window.vacationPromotionalArchive[yearVacationsAt]
        });
    }
