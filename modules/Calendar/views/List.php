...
class Calendar_List_View extends Vtiger_List_View {
	...
	function process (Vtiger_Request $request) {
		...
		$viewer->assign('VIEW', $request->get('view'));
                $viewer->assign('MOBILE', $this->mobile);
                $viewer->assign('OFFICE', $this->office);
	...
	public function initializeListViewContents(Vtiger_Request $request, Vtiger_Viewer $viewer) {
        ...
        $searchParmams = $this->SearchOfficeTest($searchParmams);
        $searchParmams = $this->SearchSourceTest($searchParmams);
		...
		if(!$this->listViewEntries){
			$this->listViewEntries = $listViewModel->getListViewEntries($pagingModel, $this->mobile, $this->office, $this->source);
		}
        ...
        function SearchOfficeTest($searchParmams){
             //echo '<pre>';print_r ($searchParmams);
            foreach($searchParmams as $key=>$value){
                if (is_array($value)){
                    $unset = $this->_SearchOfficeTest($value);
                    $searchParmams[$key] = $unset['param'];
                }
        ...
        function SearchSourceTest($searchParmams){
             //echo '<pre>';print_r ($searchParmams);
            foreach($searchParmams as $key=>$value){
                if (is_array($value)){
                    $unset = $this->_SearchSourceTest($value);
                    $searchParmams[$key] = $unset['param'];
                }
        ...
        function _SearchSourceTest($searchParmams){
            $_is = false;
            $del = false;
            foreach($searchParmams as $key=>$value){
                if (is_array($value)){
        ...
        function _SearchOfficeTest($searchParmams){
            $_is = false;
            $del = false;
            foreach($searchParmams as $key=>$value){
                if (is_array($value)){
