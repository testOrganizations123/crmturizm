...
class Calendar_Module_Model extends Vtiger_Module_Model {
	...
	*/
	...
	public static function getCalendarReminder() {
		...
		if($activityReminder != '' ) {
			...
			$reminderActivitiesResult = "SELECT reminderid, recordid FROM vtiger_activity_reminder_popup
								...
								AND ((DATE_FORMAT(vtiger_activity.due_date,'%Y-%m-%d') < ?)
								|| (DATE_FORMAT(vtiger_activity.due_date,'%Y-%m-%d') = ? and TIME_FORMAT(vtiger_activity.time_end,'%H:%i') <= ?))
                                AND (vtiger_activity.eventstatus != 'Held' AND vtiger_activity.eventstatus != 'Отказ' AND vtiger_activity.eventstatus != 'Продажа') GROUP BY recordid LIMIT 20";
                        //echo $currentUserModel->getId()."\n".$date."\n".$time."\n".$reminderActivitiesResult;
	...
	public function getOrderBySql($orderBy){
        ...
        public function getListExpressTask() {
            $listViewModel = Vtiger_ListView_Model::getInstance('Calendar');
            return $listViewModel->getListExpressTask();
        }
}
