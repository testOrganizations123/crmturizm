...
class Leads_Module_Model extends Vtiger_Module_Model {
    ...
    */
    ...
    function getHelpdeskActivities($mode, $pagingModel, $user, $recordId = false) {
		...
		$query = "SELECT c.crmid, c.smownerid, c.modifiedby,c.smcreatorid, c.modifiedtime, c.createdtime, c.description, a.cf_1085, ac.eventstatus FROM vtiger_activitycf as a INNER JOIN vtiger_seactivityrel as s ON s.activityid = a.activityid INNER JOIN vtiger_crmentity as c ON c.crmid = a.activityid LEFT JOIN vtiger_activity as ac ON ac.activityid = a.activityid";



		$query .= " WHERE ac.activitytype = 'Заявка' and c.deleted=0 and s.crmid = ?";

                $query .= " ORDER BY c.crmid DESC LIMIT ". $pagingModel->getStartIndex() .", ". ($pagingModel->getPageLimit()+1);
		...
		for($i=0; $i<$numOfRows; $i++) {
			...
			$currentUser = Users_Record_Model::getCurrentUserModel();
			$visibleFields = array('cf_1085','description','assigned_user_id','smownerid','crmid','modifiedtime','createdtime',  'eventstatus', 'smcreatorid', 'modifiedby');
			$visibility = true;
