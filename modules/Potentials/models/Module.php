...
class Potentials_Module_Model extends Vtiger_Module_Model {
	...
	function getPotentialsPipelinedAmountPerSalesPerson() {
        ...
        function getRoomsType(){
            ...
            $db = PearDatabase::getInstance();
            $sql = "Select * FROM vtiger_type_room ORDER BY type_room";
            ...
            $picklist = array();
            for ($i=0;$i<$numRows;$i++){
                $picklist[$db->query_result($result,$i,'type_roomid')] = $db->query_result($result,$i,'type_room');
            }
            return $picklist;
        }
        function getPlacementsType(){
            $db = PearDatabase::getInstance();
            $sql = "Select * FROM vtiger_placement ORDER BY placement";
            $result = $db->pquery($sql,array());
            $numRows = $db->num_rows($result);
            $picklist = array();
            for ($i=0;$i<$numRows;$i++){
                $picklist[$db->query_result($result,$i,'placementid')] = $db->query_result($result,$i,'placement');
            }
            return $picklist;
        }
        ...
        function getFoodsType(){
            $db = PearDatabase::getInstance();
            $sql = "Select * FROM vtiger_food ORDER BY food";
            $result = $db->pquery($sql,array());
            $numRows = $db->num_rows($result);
            $picklist = array();
            for ($i=0;$i<$numRows;$i++){
                $picklist[$db->query_result($result,$i,'foodid')] = $db->query_result($result,$i,'food');
            }
            return $picklist;
        }
	/**
	...
	public function getSearchRecordsQuery($searchValue, $parentId=false, $parentModule=false) {
        ...
        function getMoneyStatus($str){
            
            $array = Zend_Json::decode(htmlspecialchars_decode($str));
            if (count($array)> 0){
                foreach ($array as $value){
                    switch ($value){
                        case 'MY': return 'Моя касса'; break;
                        case 'OTV': return 'Отв. лицо'; break;
                        case 'BANK': return 'Банк (выдан кредит)'; break;
                        case 'TO': return 'Оплачено ТО'; break;
                    }
                }
            }
