...
class VChat_Commander_Action extends Vtiger_Action_Controller{
    ...
    function __construct() {
		 ...
		 $this->exposeMethod('updateDialog');
	...
	public function updateDialog($channelto,$userto,$message,$proxy,$from)
	{
		global $adb;
		...
		{
			$currentuser=$_SESSION['authenticated_user_id'];
		}
			...
			if($adb->num_rows($result) > 0) {
				for($i = 0; $i < $adb->num_rows($result); $i++) {
				   ...
				   else
				   {
					   ...
					   $this->updateCheckLogUser($currentuser); //?????
				   }
				}
				if ($proxy!=false)
				{
		...
		if ($userto!=0)
		{
			$sql = "SELECT * FROM   vtiger_vchat_text_viewsdialog WHERE userfromid='".$currentuser."' AND userid='".intval($userto)."' AND 	channelid='0' ";
			$result = $adb->pquery($sql);
			...
			{
				$sql2 = "insert into vtiger_vchat_text_viewsdialog(channelid, userfromid, userid, view_channels, datecreate,audioncheck,countmes) values(?,?,?,?,?,?,?)";
				$params2 = array(0, $currentuser, $userto, 1,  date("Y-m-d H:i:s"),0,1);
				$resultsave = $adb->pquery($sql2, $params2);
			}
	...
	public function saveMessage()
	{
			...
			if ($_REQUEST["channelto"]>0)
			{
				...
				if($adb->num_rows($result2) > 0) {
					...
					if ($subid>0)
					{
						...
						$this->updateDialog($subid,0,$_REQUEST["message"],true,0);
					}
				}	   
			}
			else
		...
		$this->updateDialog($_REQUEST["channelto"],$_REQUEST["userto"],$_REQUEST["message"],true,0);
		$result2 = array('success'=>true, 'message'=>'!!!!!!','returnid'=>'1');
		$response = new Vtiger_Response();
		$response->setResult($result2);
		$response->emit();
