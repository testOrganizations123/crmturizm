...
class Users_Record_Model extends Vtiger_Record_Model {
	...
	public function getAccessibleUsers($private="",$module = false) {
        ...
        public function getAccessibleUsers2($private="",$module = false,$office=false) {
            global $adb;
		$currentUserRoleModel = Settings_Roles_Record_Model::getInstanceById($this->getRole());
		$accessibleUser = Vtiger_Cache::get('vtiger-'.$this->getRole().'-'.$currentUserRoleModel->get('allowassignedrecordsto'), 'accessibleusers');
        if(empty($accessibleUser)) {
			...
			Vtiger_Cache::set('vtiger-'.$this->getRole().'-'.$currentUserRoleModel->get('allowassignedrecordsto'), 'accessibleusers',$accessibleUser);
                ...
                $office_accessibleUser = array();
               
                $sql = "SELECT u.id, o.officeid, o.office from vtiger_users as u LEFT JOIN vtiger_office as o ON o.officeid = u.office WHERE id in (".implode(',',array_keys($accessibleUser)).") order by o.office, u.first_name ASC";
       
                $result = $adb->pquery($sql, array());
                for ($i=0; $i < count($accessibleUser);$i++){
                    $office = $adb->query_result($result,$i, 'office');
                    $id = $adb->query_result($result,$i, 'id');
                    $office_id = $adb->query_result($result,$i, 'officeid');
                    if (empty($office)){
                        $office_accessibleUser['no']['value'][$id] = $accessibleUser[$id];
                    }
                   
                    else {
                        
                            $office_accessibleUser[$office]['value'][$id] = $accessibleUser[$id];
                            $office_accessibleUser[$office]['id'] = $office_id;
                    }
                }
                                             //   echo '<pre>';print_r($office_accessibleUser);echo '</pre>';die();
		return $office_accessibleUser;
	}
