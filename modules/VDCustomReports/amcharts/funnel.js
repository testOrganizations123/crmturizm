(function () {
	...
	e.AmSlicedChart = e.Class({
			...
			construct : function (a) {
				...
				this.colors = "#f74e41 #FF6600 #FF9E01 #FCD202 #F8FF01 #B0DE09 #04D215 #0D8ECF #0D52D1 #2A0CD0 #8A0CCF #CD0D74 #754DEB #DDDDDD #999999 #333333 #000000 #57032A #CA9726 #990000 #4B0C25".split(" ");
				this.alpha = 1;
				this.groupPercent = 0;
			...
			parseData : function () {
				...
				if (void 0 !== b) {
					...
					for (f = b = 0; f < c; f++)
						h = a[f], h.percents = h.value / d * 100, h.percents < this.groupPercent && b++;
					...
					for (f = 0; f < a.length; f++)
						c ? k = e.adjustLuminosity(c, f * d / 100) : (k = this.colors[f], void 0 === k && (k = e.randomColor())), void 0 === a[f].color && (a[f].color = k);
					this.recalculatePercents()
				}
			},
						...
						for (var z = u / 2 + t, G = (D - F) / ((E - I) / 2), C = 1, v = E / 2, E = (D - F) * (E + I) / 2 + I * F, H = g, M = 0, F = 0; F < a.length; F++) {
							...
							if (!0 !== m.hidden && (this.showZeroSlices || 0 !== m.percents)) {
								...
								if ("height" == this.valueRepresents)
									...
									if (!h && g >= y || h && g <= y)
										p = 2 * -B / I;
									else if (!h && g + p > y || h && g - p < y)
										w = h ? Math.round(p + (g - p - y)) : Math.round(p - (g + p - y)), r = w / G, B = 2 * (-B - (K - r / 2) * w) / I, Infinity != B && (p = w + B)
								}
								B = v - p / G;
								K = !1;
								...
								if (q = this.gradientRatio) {
									...
									for (x = 0; x < q.length; x++)
										r.push(e.adjustLuminosity(m.color,
												q[x]));
	...
	e.Cuboid = e.Class({
			...
			draw : function () {
				...
				if (0 < Math.abs(h) || 0 < Math.abs(k))
					if (isNaN(y))
						q = C, C = e.adjustLuminosity(G,  - .2), C = e.adjustLuminosity(G,  - .2), v = e.polygon(b, [0, h, d + h, d, 0], [0, k, k, 0, 0],
				...
				for (v = 0; v < d.length; v++)
					if (H = d[v])
						a.push(H), e.setCN(c, H, z + "element");
