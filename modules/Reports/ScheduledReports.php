...
class VtigerScheduledReport extends Reports {
	...
	public function getNextTriggerTime() {
		...
		if($scheduleType == VtigerScheduledReport::$SCHEDULED_HOURLY) {
			return date("Y-m-d H:i:s",strtotime("+1 hour"));
		}
                if ($scheduleType > 10) {
                            switch($scheduleType){
                                case 11: $_m = "+60 seconds";break;
                                case 12: $_m = "+300 seconds"; break;
                                case 13: $_m = "+600 seconds"; break;
                                case 14: $_m = "+1800 seconds"; break;
                            }
                            return date("Y-m-d H:i:s",strtotime($_m));
                        }
		if($scheduleType == VtigerScheduledReport::$SCHEDULED_DAILY) {
			return date("Y-m-d H:i:s",strtotime("+ 1 day ".$scheduledTime));
		}
		if($scheduleType == VtigerScheduledReport::$SCHEDULED_WEEKLY) {
		...
		if($scheduleType == VtigerScheduledReport::$SCHEDULED_BIWEEKLY) {
			$weekDays = array('0'=>'Sunday','1'=>'Monday','2'=>'Tuesday','3'=>'Wednesday','4'=>'Thursday','5'=>'Friday','6'=>'Saturday');
			if(date('w',time()) == $scheduledDayOfWeek) {
		...
		if($scheduleType == VtigerScheduledReport::$SCHEDULED_MONTHLY) {
			$currentTime = time();
		...
		if($scheduleType == VtigerScheduledReport::$SCHEDULED_ANNUALLY) {
		...
		if(!empty($this->scheduledRecipients)) {
			foreach($this->scheduledRecipients as $recipientType => $recipients) {
				foreach($recipients as $recipientId) {
					$selectedRecipientsHTML .= VtigerScheduledReport::generateRecipientOption($recipientType, $recipientId);
				}
			}
		}
	...
	public static function getAvailableUsersHTML() {
		...
		foreach($userDetails as $userId=>$userName) {
			$usersHTML .= VtigerScheduledReport::generateRecipientOption('users', $userId, $userName);
		}
		$usersHTML .= '</select>';
		return $usersHTML;
	}
	...
	public static function getAvailableGroupsHTML() {
		...
		foreach($grpDetails as $groupId=>$groupName) {
			$groupsHTML .= VtigerScheduledReport::generateRecipientOption('groups', $groupId, $groupName);
		}
		$groupsHTML .= '</select>';
		return $groupsHTML;
	}
	...
	public static function getAvailableRolesHTML() {
		...
		foreach($roleDetails as $roleId=>$roleInfo) {
			$rolesHTML .= VtigerScheduledReport::generateRecipientOption('roles', $roleId, $roleInfo[0]);
		}
		$rolesHTML .= '</select>';
		return $rolesHTML;
	}
	...
	public static function getAvailableRolesAndSubordinatesHTML() {
		...
		foreach($roleDetails as $roleId=>$roleInfo) {
			$rolesAndSubHTML .= VtigerScheduledReport::generateRecipientOption('rs', $roleId, $roleInfo[0]);
		}
	...
	public static function getScheduledReports($adb, $user) {
		...
		for($i=0; $i<$noOfScheduledReports; ++$i) {
			...
			$vtScheduledReport = new VtigerScheduledReport($adb, $user, $reportScheduleInfo['reportid']);
	...
	public static function runScheduledReports($adb) {
		require_once 'modules/com_vtiger_workflow/VtigerWorkflowUtils.php';
		$util = new VtigerWorkflowUtils();
