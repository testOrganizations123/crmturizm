...
Class Reports_Edit_View extends Vtiger_Edit_View {
	...
	function step2(Vtiger_request $request) {
		...
		if(!empty($record) && $primaryModule != 'sqlQuery') {
        ...
        //echo '<pre>';print_r ($primaryModuleFields);die;
        ...
        // sqlquery method
            ...
            $primaryModuleFields = $reportModel->getPrimaryModuleFieldsSqlQuery($record);
            $viewer->assign('SQL_QUERY', true);
        }
        // End
	...
	function step3(Vtiger_Request $request) {
		...
		$secondaryModuleRecordStructures = $recordStructureInstance->getSecondaryModuleRecordStructure();
                //echo '<pre>'; print_r ($primaryModuleRecordStructure);die;
        //TODO : We need to remove "update_log" field from "HelpDesk" module in New Look
		// after removing old look we need to remove this field from crm
        if($primaryModule == 'HelpDesk'){
		...
		if ($request->get('isDuplicate')) {
            ...
            $select_fields = $_POST['selected_fields']['Query'];
           
           
            $columTable = $reportModel->getColumName($request,$record);
           $viewer->assign('RECORD_ID', $record);
           
           $viewer->assign('FIELDS', base64_encode($select_fields));
		$viewer->assign('REPORT_MODEL', $reportModel);
		$viewer->assign('PRIMARY_MODULE',$primaryModule);
            $viewer->assign('SQL_QUERY', true);
            $viewer->assign('COLUM_TABLE',$columTable);
            $viewer->assign('RECORD_ID', $record);
            $viewer->assign('REPORT_MODEL', $reportModel);
            $viewer->assign('MODULE', $moduleName);
