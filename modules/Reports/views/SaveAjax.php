...
class Reports_SaveAjax_View extends Vtiger_IndexAjax_View {
	...
	public function process(Vtiger_Request $request) {
		...
		$reportModel->setModule('Reports');
                if ($reportModel->report->primodule == "sqlQuery"){
                    $select_fields = array();
                    $select_fields['inputs'] = $request->get('advanced_filter');
                    $reportModel->set('selectedFields', $select_fields);
                    
                    if ($mode === 'save') {
                        $reportModel->saveSqlQueryFilters();
                        $reportData = $reportModel->getReportData($pagingModel);
                        $data = $reportData['data'];
                    }
                    else if ($mode === 'generate') {
                      
			$query = $reportModel->getReportSQLQ($request->get('advanced_filter'),"PDF");
                        $reportData = $reportModel->getReportData($pagingModel, $request->get('advanced_filter'));
