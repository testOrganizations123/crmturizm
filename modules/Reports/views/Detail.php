	...
	function preProcess(Vtiger_Request $request) {
		...
		$secondaryModules = $reportModel->getSecondaryModules();
                 ...
                 if ($primaryModule == 'sqlQuery'){
                    ...
                    // echo '<pre>'; print_r($reportModel->getSqlQueryFiltre());
                     $viewer->assign('INPUT', $reportModel->getSqlQueryFiltre());
                     $viewer->assign('RECORD_ID', $recordId);
                     $viewer->assign('REPORT_MODEL', $reportModel);
                     $viewer->assign('COUNT',$reportData['count']);
		...
		if(empty($data)){
            ...
            $this->count = $reportModel->getReportsCount($countQuery);
                ...
                foreach ($data as $value){
                    $new_data[$value['Календарь Статус']][] = $value;
                }
                
                $_data = array();
                foreach ($new_data as $key=>$value){
                    $_data[$key] = count($value);
                }
                $sum = 0;
