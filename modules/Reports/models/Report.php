			...
			if($cachedInfo) {
				...
				if($currentUser->isAdminUser() == true || in_array($cachedInfo["owner"], $subOrdinateUsers) || $cachedInfo["owner"]==$userId) {
					...
					$this->is_editable = false;
                        ...
                        $sql = "SELECT linkid, width FROM `vtiger_links`
                                    WHERE `linktype` LIKE 'DASHBOARDWIDGET'
                                    AND `linkurl` LIKE '%name=Record&pid=$reportId%'";
                        $conect = $db->pquery($sql, array());
                        if($conect && $db->num_rows($conect)) {
					$widget = $db->fetch_array($conect);
                        }
                        else $widget = false;
