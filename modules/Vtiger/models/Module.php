...
class Vtiger_Module_Model extends Vtiger_Module {
    ...
    public function getQuickCreateFields() {
        ...
        if ($moduleName == 'FoulList'){
            foreach($fieldList as $fieldName => $fieldModel) {
                if ($fieldModel->isQuickCreateEnabled() && ($fieldModel->isEditable() || $fieldName=='users')) {
	...
	function getCalendarActivities($mode, $pagingModel, $user, $recordId = false) {
		...
		$query = "SELECT vtiger_crmentity.crmid, vtiger_crmentity.description, crmentity2.crmid AS parent_id, vtiger_crmentity.smownerid, vtiger_crmentity.setype, vtiger_activity.* FROM vtiger_activity
