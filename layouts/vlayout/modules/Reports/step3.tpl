...
{strip}
	<form class="form-horizontal recordEditView" id="report_step3" method="post" action="index.php">
		...
		<input type="hidden" name="secondary_modules" value={ZEND_JSON::encode($SECONDARY_MODULES)} />
                {if $SQL_QUERY}
                    <input type="hidden" name="selected_fields[Query]" id="seleted_fields" value='{$FIELDS}' />
                 {else}
                            ...
                            <table class="table table-bordered blockContainer showInlineTable equalSplit">
                                <tr>
                                    <th class="blockHeader" colspan="2">
                                        {vtranslate('LBL_SELECT_SQL_WHERE',$MODULE)}
                                    </th>
                                </tr>
                                {foreach key=FIELD_NAME_SQL item=FIELD_SQL from=$COLUM_TABLE}
                                    <tr>
                                        ...
                                        <td class="fieldValue large">
                                            <span class="span10">
                                                <input class="large" name="selected_fields[inputs][]" {if $FIELD_SQL['required']}required{/if} value="{$FIELD_SQL['value']}" />
                                            </span>
                                        </td>
