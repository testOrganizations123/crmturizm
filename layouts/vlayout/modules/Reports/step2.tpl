...
-->*}
{strip}
	<form class="form-horizontal recordEditView" id="report_step2" method="post" type="multipart/form-data" action="index.php">
		...
		<div class="well padding1per contentsBackground">
                    {if $SQL_QUERY}
                        <input id="sql_validate" value="1" name="sql_validate" type="hidden" />
                        <div class="row-fluid block padding1per">
                            <table class="table table-bordered blockContainer showInlineTable equalSplit">
                                <tr>
                                    <th class="blockHeader" colspan="2">
                                        {vtranslate('LBL_SELECT_SQL_QUERY',$MODULE)}
                                    </th>
                                </tr>
                                {foreach key=FIELD_NAME_SQL item=FIELD_SQL from=$PRIMARY_MODULE_FIELDS}
                                    <tr>
                                        <td class="fieldLabel small" style="width:10%">
                                            <label align="right" class="muted pull-right marginRight10px"> 
                                                {if $FIELD_SQL['required']}<span class="redColor">*</span>{/if}
                                                {$FIELD_SQL['name']}</label>
                                        </td>
                                        <td class="fieldValue large" style="width:90%">
                                            <span class="span10">
                                                <textarea class="large" name="selected_fields[{$FIELD_SQL['name']}]" {if $FIELD_SQL['required']}required{/if} style="width:100%; min-height: 350px;">{$FIELD_SQL['value']}</textarea>
                                            </span>
                                        </td>
