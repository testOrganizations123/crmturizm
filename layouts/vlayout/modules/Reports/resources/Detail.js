...
Vtiger_Detail_Js("Reports_Detail_Js",{},{
	...
	registerSaveOrGenerateReportEvent : function(){
		...
		jQuery('.generateReport').on('click',function(e){
            e.preventDefault();
                if (jQuery("#sql_validate").val() == 1){
                    var advFilterCondition = new Array();
                    var arrt = jQuery(".selected_fields");
                    jQuery.each(arrt, function() {
                        advFilterCondition.push(this.value);
                      
                    });
                    var recordId = jQuery("#sql_RECORD_ID").val();
                    
                    //console.log(arrt);
                    //var advFilterCondition = new Array();
                    //arrt.forEach(function(entry) {
                    //    console.log(entry);
                    //});
