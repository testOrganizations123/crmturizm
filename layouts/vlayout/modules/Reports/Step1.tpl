						...
						<select class="span6 chzn-select" id="primary_module" name="primary_module">
							<optgroup>
								{foreach key=RELATED_MODULE_KEY item=RELATED_MODULE from=$MODULELIST}
                                                                    {$REPORT_MODEL->getPrimaryModule()}{$RELATED_MODULE_KEY}
								...
								{/foreach}
                                                                <option value="sqlQuery" {if $REPORT_MODEL->getPrimaryModule() eq "sqlQuery" } selected="selected" {/if}>{vtranslate('LBL_SQL_QUERY','Reports')}</option>
							</optgroup>
						</select>
					</span>
				</div>
