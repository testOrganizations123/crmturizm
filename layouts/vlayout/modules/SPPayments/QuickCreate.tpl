...
<div class="modal-header contentsBackground">
    ...
    <h3>{if $ID > 0}Редактирование платежа{else}{vtranslate('LBL_QUICK_CREATE', $MODULE)} {vtranslate($SINGLE_MODULE, $MODULE)}{/if}</h3>
</div>
<form class="form-horizontal recordEditView" name="QuickCreate" method="post" action="index.php">
	...
	<input type="hidden" name="action" value="SaveAjax">
        ...
        {if $ID > 0}
        <input type="hidden" name="SaveAll" value="{$ID}">
        {/if}
			...
			<table class="massEditTable table table-bordered">
				...
				{foreach key=FIELD_NAME item=FIELD_MODEL from=$RECORD_STRUCTURE name=blockfields}
                    ...
                    {if $FIELD_MODEL->get('uitype') eq "19"}
                        ...
                        {/if}
                    {/if}
					{if $COUNTER eq 2 or $FIELD_NAME eq 'fosa_id'}
						</tr><tr>
                ...
                {assign var=office value=$FIELD_MODEL->get('fieldvalue')}
                 {else if $FIELD_NAME eq 'fosa_id'}
                     
                      {include file=vtemplate_path('uitypes/fosa.tpl',$MODULE)}
             {else}
	...
	<div class="modal-footer quickCreateActions">
		{assign var="EDIT_VIEW_URL" value=$MODULE_MODEL->getCreateRecordUrl()}
			...
			<button class="btn btn-success" onclick="SavePayment(event);return false;"><strong>{vtranslate('LBL_SAVE', $MODULE)}</strong></button>
...
{literal}
    <style>
    .massEditTable > tbody:nth-child(1) > tr:nth-child(2), .massEditTable > tbody:nth-child(1) > tr:nth-child(4), .massEditTable > tbody:nth-child(1) > tr:nth-child(5){
    display:none;
}
    </style>
{/literal}