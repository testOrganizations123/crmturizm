...
{strip}
	{foreach key=BLOCK_LABEL_KEY item=FIELD_MODEL_LIST from=$RECORD_STRUCTURE}
            {if $BLOCK_LABEL_KEY eq 'LBL_BLOCK_ANSWER_INFORMATION'}{continue}{/if}
				...
				{else}
				 ...
				 <td class="fieldLabel {$WIDTHTYPE}" id="{$MODULE_NAME}_detailView_fieldLabel_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->getName() eq 'description' or $FIELD_MODEL->get('uitype') eq '69'} style='width:8%'{/if}>
                     ...
                     {* SalesPlatform.ru end *}
						...
						{/if}
                                 ...
                                 {if $FIELD_NAME eq 'type_answer'}
                                     {assign var=type_answer value = $FIELD_MODEL->get('fieldvalue')}
                                 {/if}
				 <td class="fieldValue {$WIDTHTYPE}" id="{$MODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
					 <span class="value" data-field-type="{$FIELD_MODEL->getFieldDataType()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20' or $FIELD_MODEL->get('uitype') eq '21'} style="white-space:normal;" {/if}>
                                             {if $FIELD_NAME eq 'answer'}
                                                {include file=vtemplate_path('uitypes/answer.tpl',$MODULE_NAME) FIELD_MODEL=$FIELD_MODEL USER_MODEL=$USER_MODEL MODULE=$MODULE_NAME RECORD=$RECORD}
                                             {else if $FIELD_NAME eq 'category'}
                                                       {$FIELD_MODEL->getCategoryName($FIELD_MODEL->get('fieldvalue'))}
                                             {else if $FIELD_NAME eq 'color'}
                                                 {vtranslate($FIELD_MODEL->get('fieldvalue'), $MODULE_NAME)}
                                             {else}
                                                {include file=vtemplate_path($FIELD_MODEL->getUITypeModel()->getDetailViewTemplateName(),$MODULE_NAME) FIELD_MODEL=$FIELD_MODEL USER_MODEL=$USER_MODEL MODULE=$MODULE_NAME RECORD=$RECORD}
                                             {/if}
                                             </span>
                                         {if $MODULE_NAME eq 'Users' && ($FIELD_NAME eq 'spcompany' || $FIELD_NAME eq 'office') && !$USER_MODEL->isAdminUser() ||  $FIELD_NAME eq 'answer' ||  $FIELD_NAME eq 'type_answer'}
