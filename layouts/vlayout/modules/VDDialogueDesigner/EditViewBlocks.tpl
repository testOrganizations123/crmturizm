            ...
            {if $FIELD_MODEL->get('uitype') neq "83"}
                <td class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
                            ...
                            {if $FIELD_NAME eq 'type'}
                                <div class="row-fluid">
                                    <span class="span10">
                                {include file=vtemplate_path('uitypes/type.tpl',$MODULE)}
                                    </span>
                                </div>                                
                            {elseif $FIELD_NAME eq 'category'}
                                <div class="row-fluid">
                                    <span class="span10">
                                {include file=vtemplate_path('uitypes/category.tpl',$MODULE)}
                                    </span>
                                </div>                                
                            {elseif $FIELD_NAME eq 'color'}
                                <div class="row-fluid">
                                    <span class="span10">
                                {include file=vtemplate_path('uitypes/color.tpl',$MODULE) _name='color'}
                                    </span>
                                </div>                                
                            {elseif $FIELD_NAME eq 'type_answer'}
                                {assign var=type_answer value = $FIELD_MODEL->get('fieldvalue')}
                                <div class="row-fluid">
                                    <span class="span10">
                                {include file=vtemplate_path('uitypes/type_answer.tpl',$MODULE)}
                                    </span>
                                </div>                                
                            {elseif $FIELD_NAME eq 'answer'}
                                <div class="row-fluid" id="container-type-answer">
                                
                                {assign var=fields value=Zend_Json::decode(htmlspecialchars_decode($FIELD_MODEL->get('fieldvalue')))}
                                
                                {if count($fields) neq 0 }
                                    {include file=vtemplate_path($FIELD_MODEL->getUiAnswer($type_answer) ,$MODULE)}
                                {/if}
                                </div>
                            {else}
                                <div class="row-fluid">
