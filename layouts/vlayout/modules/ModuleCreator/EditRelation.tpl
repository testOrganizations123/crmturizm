...
<script type="text/javascript">
   function createrelation_validate(form) {
   	if(form.relatedmodule.selectedIndex == 0) {
   		alert("{/literal}{vtranslate('You have to select related module.', $MODULE)}{literal}");
   		return false;
   	}
   	if(form.relationfunction.selectedIndex == 0) {
   		alert("{/literal}{vtranslate('You have to select relation function.', $MODULE)}{literal}");
   		return false;
   	}
   	if(form.relation_label.value == '') {
   		alert("{/literal}{vtranslate('You have to set relation label.', $MODULE)}{literal}");
   		return false;
   	}
   	return true;
   }
            ...
            <div class="tab-content layoutContent padding20 themeTableColor border1px overflowVisible" style="border-radius: 0px 4px 8px;position: static !important">
               <div class="tab-pane active" id="ManageRelations">
                  <table class="settingsSelUITopLine" border="0" cellpadding="5" cellspacing="0" width="100%">
                     ...
                     <tr>
                        <td class="small" valign="top">{vtranslate('Edit relation for module', $MODULE)} {$modulename|@getTranslatedString:$MODULE}</td>
                     </tr>
                  </table>
                  <br>
                  <script type="text/javascript">
                     $(function() {
                         //var otherlists = {$otherlistsJSON};
                     
                         $("select[name='relatedmodule']").change(function(){
                         	var selectedMod = $(this).val();
                     
                         	if (selectedMod == 'Documents') {
                         		$('input[name="relationfunction"]').val('get_attachments');
                         	}
                         	else {
                         			$('input[name="relationfunction"]').val('get_related_list');
                         	}
                              ...
                              <form method="POST" action="index.php">
                                 <table class='tableHeading' cellpadding=5 cellspacing=0 border=0 width=100%>
                                    <tr class="listViewHeaders">
                                       <td class='big' colspan=2><b><font color=black>{vtranslate('Edit relation', $MODULE)}</font></b></td>
                                    </tr>
                                 </table>
                                 <table cellpadding=5 cellspacing=0 border=0 width=100%>
                                    ...
                                    <tr valign=top>
                                       ...
                                       <td class='cellText small'>
                                          <select name="relatedmodule">
                                             ...
                                             <option value="{$modname}" {if $modname eq $relatedmodule}selected="selected"{/if}>{$modname|@getTranslatedString:$MODULE}</option>
                                             {/if}
                                             {/foreach}
                                          </select>
                                          <p>
                                    ...
                                    <input type="hidden" name="relationfunction" value="{$relationfunction}" />
                                    <tr valign=top>
                                       <td class='cellLabel small'>
                                          <font color=red>*</font> <b>{vtranslate('Relation label', $MODULE)}</b>
                                       </td>
                                       <td class='cellText small'>
                                          <input type="text" class="small" name="relation_label" size="50" value="{$relation_label}">
                                          <p>
                                             {vtranslate('Type your new relation label.', $MODULE)}
                                          </p>
                                       </td>
                                    </tr>
                                 </table>
                                 <table class='tableHeading' cellpadding=5 cellspacing=0 border=0 width=100%>
                                    <tr valign=top>
                                       <td class='cellText small' colspan=2 align=right>
                                          ...
                                          <button id="Accounts_listView_basicAction_LBL_ADD_RECORD" class="btn addButton" onclick="return createrelation_validate(this.form)">
                                          <i class="icon-pencil icon-white"></i>&nbsp;<strong>{vtranslate('Edit relation', $MODULE)}</strong>
                                          </button>
                                       </td>
                                    </tr>
                                 </table>
