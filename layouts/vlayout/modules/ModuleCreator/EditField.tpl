                ...
                <div class="tab-content layoutContent padding20 themeTableColor border1px overflowVisible" style="border-radius: 0px 4px 8px;position: static !important">
                    ...
                    <div class="tab-pane active" id="ManageRelations">
                        <table class="settingsSelUITopLine" border="0" cellpadding="5" cellspacing="0" width="100%">
                            ...
                            <tr>
                                <td class="small" valign="top">{vtranslate('Edit related field for module', $MODULE)} {$modulename|@getTranslatedString:$MODULE}</td>
                            </tr>
                        </table>
...
<table class="table table-bordered themeTableColor" border="0" cellpadding="10" cellspacing="0" width="100%">
                                        ...
                                        <form method="POST" action="index.php">
                                            ...
                                            <table class='tableHeading' cellpadding=5 cellspacing=0 border=0 width=100%>
                                                <tr class="listViewHeaders">
                                                    <td class='big' colspan=2><b><font color=black>{vtranslate('Edit field', $MODULE)}</font></b></td>
                                                </tr>
                                            </table>
							...
							<td class='cellText small'>
								<select name="blockid">
									...
									{if $modinfo.presence eq 0}
										<option value="{$block.blockid}" {if $block.blockid==$blockid}selected="selected"{/if}>{$block.blocklabel|@getTranslatedString:$modulename}</option>
									{/if}
									{/foreach}
								</select>
								<p>
						...
						<tr valign=top>
							...
							<td class='cellText small'>
								<select name="relatedmodule">
									...
									{if $modinfo.presence eq 0}
										<option value="{$modname}" {if $modname==$relatedmodule}selected="selected"{/if}>{$modname|@getTranslatedString:$MODULE}</option>
									{/if}
									{/foreach}
								</select>
								<p>
    ...
    <i class="icon-pencil icon-white"></i>&nbsp;<strong>{vtranslate('Edit field', $MODULE)}</strong>
</button>
