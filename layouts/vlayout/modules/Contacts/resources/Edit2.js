...
jQuery('document').ready(function(){
    jQuery('input[name="lastname"]').on('keyup', function(){
        var str = jQuery(this).val();
        jQuery('input[name="firsname_latin"]').val(str.translit());
    });
     jQuery('input[name="firstname"]').on('keyup', function(){
         var str = jQuery(this).val();
        jQuery('input[name="lastname_latin"]').val(str.translit());
    });
...
function saveAjax(){
    ...
    var form = jQuery('#contactCreate');
    if (form.find('input[name="lastname"]').val() == ""){
        form.find('input[name="lastname"]').attr('required','required').focus();
        return false;
    };
    if (form.find('input[name="firstname"]').val() == ""){
        form.find('input[name="firstname"]').attr('required','required').focus();
        return false;
    };
     if (form.find('input[name="firsname_latin"]').val() == ""){
        form.find('input[name="firsname_latin"]').attr('required','required').focus();
        return false;
    };
      if (form.find('input[name="lastname_latin"]').val() == ""){
        form.find('input[name="lastname_latin"]').attr('required','required').focus();
        return false;
    };
      if (form.find('input[name="birthday"]').val() == ""){
        form.find('input[name="birthday"]').attr('required','required').focus();
        return false;
    };
     if (form.find('input[name="typevisa"]').val() == ""){
        form.find('input[name="typevisa"]').attr('required','required').focus();
        return false;
    };
    ...
    AppConnector.request(quickCreateSaveUrl).then(
                function(data) {
                    console.log(data.result._recordId);
                    var _data = [data.result._recordId];
                    var url = {data :
                                    {
                            data: _data,
                            module: 'Potentials',
                        ...
                        AppConnector.request(url).then(function(data){
                            jQuery('tr.addturist').before(data);
                            jQuery(".blockUI.blockOverlay").remove();
                             jQuery(".blockUI.blockMsg.blockPage").remove();
