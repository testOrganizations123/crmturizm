...
{strip}
    ...
    {foreach key=key_air item=delta from=$ARRAY_FIELD_VALUE} 
        <div class="row-fluid flite" {if $key_air eq 0}style="margin-top: 25px;"{/if}>
    <div class="span3">
        <div class="form-group">
             {assign var=FIELD_MODEL value=$BLOCK_FIELDS['addservise']}
                           ...
                           {if $key_air eq 0} <label for="{$FIELD_MODEL->getFieldName()}">{vtranslate($FIELD_MODEL->get('label'), $MODULE)} {if $FIELD_MODEL->isMandatory() eq true} <span class="redColor">*</span> {/if}</label> {/if}
                                ...
                                {if $opportunity_type eq "Пакетный Тур" || $opportunity_type eq "Индивидуальный Тур"}
                                
                                <select class='form-control {if $opportunity_type eq ""}hide{/if} turPaket addShow  {if $OCCUPY_COMPLETE_WIDTH} row-fluid-fluid {/if}' name='{$FIELD_MODEL->getFieldName()}[]'  {if $FIELD_MODEL->isMandatory() eq true} required{/if} name="{$FIELD_MODEL->getFieldName()}" data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'>
                           ...
                           {else if $opportunity_type eq "Авиа билеты"}
                               <select class='form-control  ticket addShow {if $OCCUPY_COMPLETE_WIDTH} row-fluid-fluid {/if}' name='{$FIELD_MODEL->getFieldName()}_ticket[]'  {if $FIELD_MODEL->isMandatory() eq true} required{/if} name="{$FIELD_MODEL->getFieldName()}" data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'>
                                   <option value=''>{vtranslate('LBL_SELECT_OPTION','Vtiger')}</option>
                            ...
                            {else if $opportunity_type eq "ЖД билеты"}
                                   <select class='form-control rail addShow  {if $OCCUPY_COMPLETE_WIDTH} row-fluid-fluid {/if}' name='{$FIELD_MODEL->getFieldName()}_rail[]'  {if $FIELD_MODEL->isMandatory() eq true} required{/if} name="{$FIELD_MODEL->getFieldName()}" data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'>
                                   <option value=''>{vtranslate('LBL_SELECT_OPTION','Vtiger')}</option>
...
<div class="span2">
                           ...
                           {if $key_air eq 0} <label for="{$FIELD_MODEL->getFieldName()}">{vtranslate($FIELD_MODEL->get('label'), $MODULE)} {if $FIELD_MODEL->isMandatory() eq true} <span class="redColor">*</span> {/if}</label>{/if}
                                ...
                                <select class='  form-control {if $OCCUPY_COMPLETE_WIDTH} row-fluid-fluid {/if}' name='{$FIELD_MODEL->getFieldName()}[]'  {if $FIELD_MODEL->isMandatory() eq true} required{/if} name="{$FIELD_MODEL->getFieldName()}" data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'>
                                    ...
                                    <option value="{$KEY}" {if $FIELD_VALUE.$key_air eq $KEY}selected{/if}>{$KEY}</option>
                                    {/foreach}
                                    </select>
    ...
    <div class="span2 rail ticket addShow">
                                ...
                                {assign var="FIELD_VALUE" value=$FIELD_MODEL->get('fieldvalue')}
                                {assign var="FIELD_VALUE" value=explode('#', $FIELD_VALUE)}
                           {if $key_air eq 0} <label for="{$FIELD_MODEL->getFieldName()}">{vtranslate('Направление', $MODULE)} {if $FIELD_MODEL->isMandatory() eq true} <span class="redColor">*</span> {/if}</label>{/if}
                                ...
                                <select class='addServiseCalculate  form-control {if $OCCUPY_COMPLETE_WIDTH} row-fluid-fluid {/if}' name='{$FIELD_MODEL->getFieldName()}[]'  {if $FIELD_MODEL->isMandatory() eq true} required{/if} name="{$FIELD_MODEL->getFieldName()}" data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'>
                                    ...
                                    <option value="" {if $FIELD_VALUE[$key_air] eq ''}selected{/if}>Выберите направление</option>
                                    <option value="1" {if $FIELD_VALUE[$key_air] eq 1}selected{/if}>Туда</option>
                                    <option value="2" {if $FIELD_VALUE[$key_air] eq 2}selected{/if}>Обратно</option>
                                    <option value="3" {if $FIELD_VALUE[$key_air] eq 3}selected{/if}>Туда и обратно</option>
    ...
    <div class="span4 rail ticket addShow" style="width:305px">
                         ...
                         {if $key_air eq 0}   <label for="{$FIELD_MODEL->getFieldName()}">{vtranslate('Пасажиры', $MODULE)} {if $FIELD_MODEL->isMandatory() eq true} <span class="redColor">*</span> {/if}</label>{/if}
                           
                                
                                <select class='addServiseCalculate chzn-select form-control {if $OCCUPY_COMPLETE_WIDTH} row-fluid-fluid {/if} listTurist' multiple name='{$FIELD_MODEL->getFieldName()}[{$key_air}][]'  {if $FIELD_MODEL->isMandatory() eq true} required{/if} name="{$FIELD_MODEL->getFieldName()}" data-selected-value='{$FIELD_MODEL->get('fieldvalue')}' style="width:300px;">
