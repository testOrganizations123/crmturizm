...
{strip}
    {assign var=LIST_PAYMENT value=$RECORD_STRUCTURE_MODEL->getListPayment($RECORD_ID,'Receipt')}
...
{foreach key=record_payment item=PAYMENT from=$LIST_PAYMENT}
   <div class="row-fluid" id="payment-id-{$record_payment}">
    ...
    <div class="span2">
        <div class="form-group form-group-required">
            <label>Статус денег <i class="fa fa-check"></i></label>
                                {assign var=FIELD_MODEL value=$BLOCK_FIELDS['cf_1270']}
                                {assign var="FIELD_INFO" value=Vtiger_Util_Helper::toSafeHTML(Zend_Json::encode($FIELD_MODEL->getFieldInfo()))}
                                {assign var="SPECIAL_VALIDATOR" value=$FIELD_MODEL->getValidator()}
                                {assign var="FIELD_VALUE" value=$FIELD_MODEL->get('fieldvalue')}
                                
                             {assign var="FIELD_VALUE" value=Zend_Json::decode(htmlspecialchars_decode($FIELD_VALUE))}
             
            <select class="form-control" name="cf_1270[{$record_payment}]">
                                {if $FIELD_VALUE.$record_payment eq ""} <option value="">&nbsp;</option>{/if}
                                <option value="MY" {if $FIELD_VALUE.$record_payment eq "MY"}selected{/if}>Моя касса</option>
                                <option value="OTV" {if $FIELD_VALUE.$record_payment eq "OTV"}selected{/if}>Отв. лицо</option>
                                <option value="BANK" {if $FIELD_VALUE.$record_payment eq "BANK"}selected{/if}>Банк (выдан кредит)</option>
                                <option value="TO" {if $FIELD_VALUE.$record_payment eq "TO"}selected{/if}>Оплачено ТО</option>
                            </select>
            
        </div>
        {assign var=listNumber value = $listNumber+1}    
    </div>
