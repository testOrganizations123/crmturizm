...
<select {if $amoun_cur > 0}disabled{/if} class='form-control   {if $OCCUPY_COMPLETE_WIDTH} row-fluid {/if}' name='{$FIELD_MODEL->getFieldName()}' {if $FIELD_MODEL->isMandatory() eq true} required{/if} data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'>
