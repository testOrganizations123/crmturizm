				...
				<span class="btn-toolbar span4">
					...
					{/foreach}
                                        <span class="btn-group">
                                            <button id="leads_mass_transfer" class="btn addButton" onclick="Potentials_mass_transfer(event)"><i class="icon-refresh"></i>&nbsp;<strong>Передать Бронь</strong></button>
                                        </span>
				</span>
			<span class="btn-toolbar span4">
						...
						<select id="customFilter" style="width:350px;">
							...
							<optgroup label=' {if $GROUP_LABEL eq 'Mine'} &nbsp; {else if} {vtranslate($GROUP_LABEL)} {/if}' >
									{foreach item="CUSTOM_VIEW" from=$GROUP_CUSTOM_VIEWS}
                                                                            {if $CUSTOM_VIEW->get('cvid') eq 10} {continue}{/if}
