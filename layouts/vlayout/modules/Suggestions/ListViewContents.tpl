		...
		<tr class="listViewEntries"  data-id='{$LISTVIEW_ENTRY->getId()}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
			...
			<td class="listViewEntryValue {$WIDTHTYPE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}" nowrap>
				...
				{else if $LISTVIEW_HEADER->get('uitype') eq '72'}
					...
					{/if}
                                {else if $LISTVIEW_HEADERNAME eq 'script'}
                                    <a href="index.php?module=VDDialogueDesigner&view=RunScript&record={$LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME)}" target="_blank">Перейти на скрипт</a>
                                    &nbsp;&nbsp;||&nbsp;&nbsp;
                                    <a href="index.php?module=VDDialogueDesigner&view=Edit&record={$LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME)}" >Открыть коструктор</a>
				{else}
