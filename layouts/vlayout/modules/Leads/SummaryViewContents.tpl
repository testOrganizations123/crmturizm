			...
			<tr class="summaryViewEntries">
				...
				<td class="fieldValue" style="width:65%">
                    <div class="row-fluid">
                        ...
                        {if $FIELD_MODEL->isEditable() eq 'true' && ($FIELD_MODEL->getFieldDataType()!=Vtiger_Field_Model::REFERENCE_TYPE) && $IS_AJAX_ENABLED && $FIELD_MODEL->isAjaxEditable() eq 'true' && $FIELD_MODEL->get('uitype') neq 69 && !$READONLY}
                            {$FIELD_MODEL->get('fieldvalue')  {if ($FIELD_NAME eq 'rating' and $FIELD_MODEL->get('fieldvalue') eq 'Скидочник') or ($FIELD_NAME eq 'leadstatus' and trim($FIELD_MODEL->get('fieldvalue')) eq 'Готов купить тур')  }
