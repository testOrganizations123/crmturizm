...
<br>
<div class="listViewEntriesDiv listViewPageDiv" style='overflow-x:auto;'>
			...
			<input type="hidden" name="deleteActionUrl" value="{$LISTVIEW_ENTRY->getDeleteUrl()}">
				...
				{elseif $LISTVIEW_HEADER->getName() neq 'last_name' and $LISTVIEW_HEADER->getName() neq 'email1'}
					...
					<td class="{$WIDTHTYPE}" nowrap>
                                             {if $LISTVIEW_HEADERNAME eq 'office'}
                                                         {include file='uitypes/officeList.tpl'|@vtemplate_path:$MODULE listvalue=$LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME)}
                                                     {else}{vtranslate($LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME), $MODULE)}{/if}
