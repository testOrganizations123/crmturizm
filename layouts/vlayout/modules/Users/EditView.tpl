		...
		<div>
			{foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE}
				...
				{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
                                        {if (!$USER_MODEL->isAdminUser() && $USER_MODEL->get('add_user' neq 1)) && ($FIELD_NAME eq 'spcompany' || $FIELD_NAME eq 'office' || $FIELD_NAME eq 'add_user')}{continue}{/if}
                                         {if (!$USER_MODEL->isAdminUser()) && ($FIELD_NAME eq 'add_user' || $FIELD_NAME eq 'is_admin')}{continue}{/if}
