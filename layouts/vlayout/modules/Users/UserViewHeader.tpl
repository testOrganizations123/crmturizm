                ...
                <div class="span6">
                    <div class="row-fluid pull-right detailViewButtoncontainer">
						<div class="btn-toolbar pull-right">
							...
							{/if}
                                                        {assign var=privelege value=explode('::',$CURRENT_USER_MODEL->getParentRoleSequence())}
                                                         {if count($privelege) < 5}
                                                             <span class="btn-group">
                                                                 <button id="leads_mass_transfer" class="btn addButton" onclick="Paritet_mass_transfer(event)"><i class="icon-refresh"></i>&nbsp;<strong>Паритетная передача</strong></button>
                                                             </span>
							{/if}
                        </div>
                    </div>
