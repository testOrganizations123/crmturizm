...
<div class="contents-topscroll">
    <div class="topscroll-div">
        &nbsp;{if ($MODULE eq 'ListCountry' or $MODULE eq 'TypeTourisVisa' or $MODULE eq 'ListTouroperators') and $bottom_copy}
                <div>Скопировать из: <select id='country_list'>
            {foreach key=id item =country from=$bottom_copy}
                <option value="{$id}">{$country}</option>
                {/foreach}
            </select> &nbsp; <a href="#" onclick="relatedCopy()">Скопировать</a></div>
        {/if}
    </div>
</div>
