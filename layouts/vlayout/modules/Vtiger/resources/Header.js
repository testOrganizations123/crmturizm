...
//        }
    ...
    registerEvents: function() {
        ...
        jQuery('#quickCreateModules,#compactquickCreate,#createFoul').on("click", ".quickCreateModule", function(e, params) {
            if (typeof params == 'undefined') {
                params = {};
            }
    ...
    AppConnector.request(params).then(
				function(data){
                                   ...
                                   if (data.success === true){
                                       var param = {data:data.result, css:{'top':"100px"}};
...
function transfer_complite(event){
    event.preventDefault();
    var id={};
    var data = [];
    jQuery('.radioCrmid:radio:checked').each(function(){
        
        var key = jQuery(this).data('id');
        var id = { value: jQuery(this).val(), crmid: key};
        data.push(id);
    });
...
function Leads_mass_transfer(event){
    event.preventDefault();
    var id = [];
    ...
    if (id.length === 0){
        alert('Нет выбраных заявок для передачи');
    }
    else {
        var user = jQuery('#selectUserTransfer').val();
        console.log(user);
     ...
     var progressIndicatorInstance = jQuery.progressIndicator({});
        ...
        var params = { 
				...
				function(data){
                                    app.hideModalWindow()
                                    progressIndicatorInstance.hide(); 
                                    var param = {text:"Заявки приняты на передачу, дождитесь приема со стороны менеджера"};
...
function Potentials_mass_transfer(event){
    event.preventDefault();
    var id = [];
    jQuery('.listViewEntriesCheckBox:checkbox:checked').each(function(){
        id.push(jQuery(this).val());
    ...
    if (id.length === 0){
        alert('Нет выбраных броней для передачи');
    }
    else {
        var user = jQuery('#selectUserTransfer').val();
        console.log(user);
     ...
     var progressIndicatorInstance = jQuery.progressIndicator({});
        ...
        var params = { 
				...
				function(data){
                                    app.hideModalWindow()
                                    progressIndicatorInstance.hide(); 
                                    var param = {text:"Брони приняты на передачу, дождитесь приема со стороны менеджера"};
                                    Vtiger_Helper_Js.showPnotify(param);
...
function Paritet_mass_transfer(event){
    event.preventDefault();
...
function paritet_complite(event){
    event.preventDefault();
    ...
    jQuery('.radioCrmid:radio:checked').each(function(){
     ...
     var params = {
        ...
        action: 'Transfer',
        mode: 'complitParitet',
        userid: jQuery("input#recordId").val(),
        record : data,
    ...
    AppConnector.request(params).then(
				function(data){
                                   app.hideModalWindow()
                                    progressIndicatorInstance.hide(); 
                                    var param = {text:"Сущности перенесены"};
                                    Vtiger_Helper_Js.showPnotify(param);
                                   });
}
