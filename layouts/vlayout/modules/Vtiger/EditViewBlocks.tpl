		...
		{/if}
            ...
            {if $FIELD_MODEL->get('uitype') neq "83"}
                <td class="fieldValue {$WIDTHTYPE}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
                    <div class="row-fluid">
                        <span class="span10">
                            {if $FIELD_NAME eq 'typevisa'}
                                {include file=vtemplate_path('uitypes/typeturvisa.tpl',$MODULE) BLOCK_FIELDS=$BLOCK_FIELDS}
                                 {else if $FIELD_NAME eq 'office' && $MODULE neq 'Office'}
                {include file=vtemplate_path('uitypes/office.tpl',$MODULE)}
                {assign var=office value=$FIELD_MODEL->get('fieldvalue')}
                 {else if $FIELD_NAME eq 'fosa_id'}{$office}
